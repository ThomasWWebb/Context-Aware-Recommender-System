...
def recommend(activeUserID, userContext, ratings, contexts, contextValues):
    ...
    allSeenItems = ratings[(ratings.UserID == activeUserID) & (ratings.mood == userContext)]
    ...
    similarityTuples = []
    for context in contexts:
        contextRatings = ratings[ratings['mood'] == context]
...
def addMoods(ratings, contexts):
    for index in range(ratings['ItemID'].size):
        if ratings.at[index, 'sleepiness'] == "sleepy" and pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = "lazy"
        elif ratings.at[index, 'sleepiness'] == "awake" and pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = "active"
        elif ratings.at[index, 'DrivingStyle'] == "relaxed driving" and pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = "lazy"
        elif ratings.at[index, 'DrivingStyle'] == "sport driving" and pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = "active"
        elif ratings.at[index, 'trafficConditions'] == "traffic jam" and pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = "sad"
        elif ratings.at[index, 'trafficConditions'] == "free road" and pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = "happy"
        elif pd.isnull(ratings.at[index, 'mood']):
            ratings.at[index, 'mood'] = random.choice(contexts)

    ratings = ratings.drop(columns=['sleepiness', 'DrivingStyle', 'trafficConditions'])
...
def runEvaluation(originalRatings, contexts, contextValues, listSize, likeThreshold):
    ...
    for train_index, test_index in kf.split(originalRatings):
        ...
        for userID, row in groupTest:
            ...
            #group by mood
            userGroup = groupTest.get_group(userID).groupby('mood')
            userSum += 1
...
def displayRecommendations(recommendations, musicTracks):
    ...
    for index in range(10):
        top10.append(recommendations[index][0])
        top10Ratings.append(recommendations[index][1])
    top10Ratings = np.array(top10Ratings)
...
def recommendForUser(userID, ratings, contexts, contextValues, musicTracks):
    ...
    while choice != "x":
        ...
        choice = input()
        if choice == "1":
            recommendations = recommend(userID, "happy" ,ratings, contexts, contextValues)
            displayRecommendations(recommendations, musicTracks)
        elif choice == "2":
            recommendations = recommend(userID, "sad" ,ratings, contexts, contextValues)
            displayRecommendations(recommendations, musicTracks)
        elif choice == "3":
            recommendations = recommend(userID, "lazy" ,ratings, contexts, contextValues)
            displayRecommendations(recommendations, musicTracks)
        elif choice == "4":
            recommendations = recommend(userID, "active" ,ratings, contexts, contextValues)
            displayRecommendations(recommendations, musicTracks)
        os.system('cls')
        
        

contexts = ["sad", "lazy", "happy", "active"]
contextValues = [[1, 2, 4, 2], 
                [2, 1, 2, 4], 
                [4, 2, 1, 2],
                [2, 4, 2, 1]]
...
ratings = addMoods(ratings, contexts)
...
originalRatings = ratings.drop_duplicates(subset=["UserID", "ItemID", "mood"])
